    ...
    <div className="h-full rounded-4xl bg-background-secondary/40 backdrop-blur-md border border-border-subtle p-8 shadow-sm">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-sm font-medium text-muted">{requests.length} submitted</p>
        </div>
      </div>
      ...
      {requests.length > 0 ? (
          ...
          {requests.map((r) => (
            ...
            >
              <div className="flex items-start justify-between gap-4 mb-4">
                <h4 className="font-bold text-foreground text-lg leading-tight flex-1">{r.title}</h4>
                {r.completed ? (
              ...
              <div className="flex items-center gap-2 bg-background-secondary/50 w-fit px-3 py-1.5 rounded-lg border border-border-subtle">
                ...
                <span className="text-sm font-bold text-foreground">{r.votes.length} <span className="text-muted font-medium">votes</span></span>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="text-center py-16 rounded-2xl border border-dashed border-border-subtle bg-background/50">
          ...
          <h4 className="font-bold text-lg mb-1 text-foreground">No requests yet</h4>
          <p className="text-sm text-muted">Suggest a new problem or feature!</p>
        </div>
      )}
    </div>
  );
