...
export default function MyReports({ reports }: { reports: MyReport[] }) {
  return (
    <div className="rounded-4xl bg-background-secondary/40 backdrop-blur-md border border-border-subtle p-8 md:p-10 shadow-sm">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h3 className="text-2xl font-bold text-foreground">My Bug Reports</h3>
          <p className="text-sm font-medium text-muted">{reports.length} issues submitted</p>
        </div>
      </div>
      ...
      {reports.length > 0 ? (
          ...
          {reports.map((r) => (
            ...
            >
              <div>
                ...
                <h4 className="font-bold text-foreground text-lg leading-tight mb-4">{r.title}</h4>
              </div>
              ...
              >
                <span className="truncate max-w-50">{r.problemId.title}</span>
        ...
        <div className="text-center py-16 rounded-2xl border border-dashed border-border-subtle bg-background/50">
          ...
          <h4 className="font-bold text-lg mb-1 text-foreground">No reports yet</h4>
          <p className="text-sm text-muted">You haven&apos;t reported any issues with problems. Awesome!</p>
        </div>
      )}
    </div>
  );
